...
void SetUp3DObject()
{
	...
	int width, height, nrChannels;
	unsigned char *data = stbi_load("./textures/RAMO_transparent.png", &width, &height, &nrChannels, 0);
	...
	// rotate in X access
	model = glm::rotate(model, glm::radians(angleX), glm::vec3(1.0, 0.0, 0.0));
	// rotate in Y access
	model = glm::rotate(model, glm::radians(angleY), glm::vec3(0.0, 1.0, 0.0));
	// rotate in Z access
	model = glm::rotate(model, glm::radians(angleZ), glm::vec3(0.0, 0.0, 1.0));
