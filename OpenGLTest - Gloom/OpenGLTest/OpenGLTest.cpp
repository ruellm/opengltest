...
int main()
{
	...
	GLShaderFx brightnessFilter;
	brightnessFilter.Init();
	brightnessFilter.LoadVertexShaderFromFile("./shaders/model_vshader.txt");
	brightnessFilter.LoadFragmentShaderFromFile("./shaders/brightness_filter_fshader.txt");
	brightnessFilter.Build();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		color2.Bind();
			...
			gWVP = brightnessFilter.GetUniformLocation("gVP");
			gWorld = brightnessFilter.GetUniformLocation("gWorld");
			alphaID = brightnessFilter.GetUniformLocation("alpha");

			brightnessFilter.SetUniformMatrix(gWVP, projectionMatrix * viewMatrix);
			brightnessFilter.SetUniformMatrix(gWorld, model);
			brightnessFilter.SetUniformFloat(alphaID, alpha);

			textureID = brightnessFilter.GetUniformLocation("gSampler");
			glUniform1i(textureID, 0);				// set the textureID to texture unit 0
			glActiveTexture(GL_TEXTURE0);			// activate the texture unit 0 first before binding texture
